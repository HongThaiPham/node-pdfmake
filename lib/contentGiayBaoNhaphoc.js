...
const getNganhTable = (data) => {
  ...
  if (data && data.length > 0) {
    dataTable = data.map((x, idx) => {
      return [idx + 1, x.NganhTrungTuyen, x.MaNganh, x.TongDiem, x.PhuongThuc];
    });
  }
  return dataTable;
};
...
const getHocPhiTable = (data) => {
  ...
  if (data && data.length > 0) {
    data.forEach((element) => {
      dataTable.push({
        margin: [15, 0, 0, 0],
        text: `- Học phí ngành ${element.NganhTrungTuyen}: ${element.HocPhi} đồng/1 học kỳ`,
      });
      if (element.LoaiHB) {
        dataTable.push({
          margin: [15, 0, 0, 0],
...
const buildContent = (data) => {
  ...
  return [
    ...
    // },
    {
      margin: [15, 0, 0, 0],
      text:
        "1 - Giấy báo nhập học do Hội đồng tuyển sinh Nhà trường cấp (tờ giấy này).",
    },
    {
      margin: [15, 0, 0, 0],
      text:
        "2 - Giấy chứng nhận tốt nghiệp tạm thời hoặc Bằng tốt nghiệp THPT (bản sao có công chứng) và học bạ THPT (bản sao có công chứng). Nếu chưa có giấy chứng nhận tốt nghiệp tạm thời thì có thể bổ sung sau.",
    },
    {
      margin: [15, 0, 0, 0],
      text:
        "3 - Giấy khai sinh hoặc chứng minh nhân dân (bản sao có công chứng).",
    },
    // {
    //   margin: [15, 0, 0, 0],
    ...
    ...hocPhiNguyenVong,
    {
      margin: [15, 0, 0, 0],
      text:
        "(Cam kết không tăng học phí trong suốt quá trình học. Một năm có 2 học kỳ. Học phí có thể đóng linh động nhiều đợt).",
    },
